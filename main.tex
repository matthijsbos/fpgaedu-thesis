...
\caption{The basic model, an abstract overview of the FPGA development board and its exposed interfaces.}
...
\caption{The basic model, an abstract overview of the FPGA and its contained experiment setup logic. No specific architecture is applied to the logic.}
...
\caption{I/O virtualized, an abstract overview of the FPGA and its contained logic architecture. The controller embeds the experiment setup logic into the FPGA.}
...
\caption{I/O virtualized, an abstract overview of the FPGA development board and its exposed interfaces.}
...
\caption{Cycle control, an abstract overview of the FPGA and its contained logic architecture. The \texttt{clock\_enable} and \texttt{reset} signals are added to allow for cycle-accurate interaction and (re)initialization.}
...
\begin{figure}[h]
    ...
    %TODO hier staat opeens een d-flipflop, waarom?
\end{figure}
...
\caption{Cycle control, an abstract overview of the FPGA development board. The controller's interface is extended with operations that allow for cycle-accurate interaction with the experiment setup.}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[t]{0.5\textwidth}
        \centering
        \includegraphics[height=1.5in]{interface-bram}
        \caption{Single-port block RAM}
        \label{fig:clock-manipulation-enable}
    \end{subfigure}%
    \begin{subfigure}[t]{0.5\textwidth}
        \centering
...
% het is nog niet mogelijk te interfacen met complexe state devices in de experiment setup, zoals register arrays en memory elements. 
...
% Removing the dependency from controllerto experiment, generic interface.
...
The controller will serve as an intermediary between the pc software and the experiment setup. In betewen experiment cycles, the controller will handle user operations and 

% \begin{figure}
% \centering
% \includegraphics[width=\textwidth]{processes-dependencies-abstract}
% \caption{Controller abstracted, Process dependency graph}
% \label{fig:processes-dependencies-abstract}
% \end{figure}

% \begin{figure}
% \centering
% \makebox[\textwidth][c]{\includegraphics[width=1.25\textwidth]{processes-abstract}}
% \caption{Controller abstracted, Processes overview}
% \label{fig:processes-abstract}
% \end{figure}

% \begin{figure}
% \centering
% \includegraphics[width=\textwidth]{toolchain-abstract}
% \caption{Caption}
% \label{fig:my_label}
% \end{figure}
