...
% Ook de onderstaande punten moeten worden losgekoppeld van de moethode en alleen worden toegepast op de inzet van FPGA's in onderwijs als een fysieke component.
...
% De docenten willen bijvoorbeeld dat studenten in staat zijn een instructie abstract te implementeren op RTL level, maar zijn niet langer geinteresseerd studenten in detail te leren hoe je een full adder implementeert.
...
% behandelen dat de docenten voorzien dat FPGA's op een aantal vlakken het onderwijs kunnen bevorderen: 
% Om het begrip te vergroten en toch een bepaalde intuitie voor lower-level zaken te ontwikkelen zien de docenten een kans voor FPGA's. Door deze zaken middels een fysieke component tastbaar te maken wordt deze intuitie sneller ontwikkeld. 

% De complexe en abstracte stof vertalen naar de fysieke werkelijkheid. 

% Een beter begrip van de randzaken ontwikkelen. Een fysieke component helpt in het creeren van een overall bewustzijn.  

% Deze verwachtingen ondersteunen door de resultaten uit de papers
...
% - gebrek aan diepgang, simpele implementaties
% - gebrek aan diepgang, geen analyse
% - gebrek aan diepgang door complexiteit van ontwikkelproces
% - meer gericht op ouderjaars studenten die soms al ervaren zijn met ten minste één van de onderwerpen.
% - Leerdoelen sluiten niet aan bij die van de docenten. HDL, fpga development en digital design valt buiten het curriculum van CS. Veel van de skills die nodig zijn om zelfstandig tot implementatie te komen zijn irrelevant voor CS studenten.
% - Tools zijn niet gericht op verzamelen van resultaten.
...
% The UvA's course instructors believe that these methods suffice in the teaching of basic computer architecture concepts, but are insufficient in explaining more complex subjects to students.

% TODO sterker benadrukken dat het sterk ontbreekt aan diepgang op een hoger abstract niveau. Het is voor digital design purposes een mooie methode, maar niet om complexe designvraagstukken uit te leggne.

% CS studenten hebben er niets aan om ervaring op te doen met allerlei EDA tools

% in \cite{lee2012pipelined} zeggen ze juist dat een benadering zonder implementatie studenten demotiveert

% Students are required to think on multiple levels of abstraction simultaneously, increasing the complexity. The time spent in development and testing is lost on addressing higher-level subjects.
...
% ###############################
% #### hier een stuk welke methoden meer het doel van de docenten nastreven
% ################################
...
% TODO soms zijn de practica juist beperkt tot instruction level en niet lager. De architectuur is fixed en stelt studenten niet in staat design decisions op RTL niveau te evalueren.

% TODO deze methoden vereisen veel werk van de docenten. vereisen een grote investering op meerdere vlakken die zich slecht laat terugbetalen

% TODO de huidige ontwikkelmethoden en gereedschappen zijn niet gericht het abstraheren van alle complexiteit. Je moet eerst alles weten voordat je aan de slag kunt, zowel als student en docent. Docenten ontwikkelen vaak experimenten, studenten maken gebruik van de zelfde complexe en omvangrijke tools om het experiment uit te voeren.

% om te bereiken wat de docenten willen is het erg omslachtig om experimenten te ontwikkelen. Het is veel werk om een experiment te ontwikkelen. Er zijn geen standaarden of standaardcomponenten die dit vergemakkelijken. Het delen van experimenten is erg moeilijk omdat de experimenten vaak specifiek gebouwd zijn voor één specifiek bordje. Een experiment aanpassen aan nieuwe hardware vereist vervolgens weer veel werk.  Ook is bij een docent veel kennis vereist voor het ontwikkelen van een dergelijk experiment. Dat maakt dat het dat de ontwikkeling van een experiment op dit moment een grote investering in tijd en kennis vereist. DONT REPEAT YOURSELF
...
% studenten moeten op een zeer envoudige manier in staat zijn het experiment te observeren en interactie te hebben met het experiment. De bovenstaande methoden laten dit niet toe en leiden de student alleen maar heel erg af van het doel van het experiment, doordat ze op verschillende abstractieniveaus actief moeten zijn. 


% TODO uitleggen dat de docenten juist diepgang willen creeren door studenten te laten experimenteren met een breed scala aan architecturen. Bij wijze van spreke alle stappen die H&P doorlopen in hun proces implementeren en de student in staat stellen elke stap live mee te kunnen kijken.

% Uiteindelijk is dus het onderliggende probleem dat het ontbreekt aan een methode waarbij studenten in staat wordt gesteld m.b.v. een fysieke component te experimenteren met een wijd scala aan architecturen, zodat voldoende diepgang kan worden gecreerd. Zij moeten hierin niet worden gehinderd door een enorme berg aan voorwerk en vereiste skills. 

% Voor docenten moet het makkelijk worden om een variatie aan ontwerpen te creeren zonder dat zij elke keer het wiel moeten uitvinden. Bovendien is het voor de overdraagbaarheid essentieel dat ze niet zijn beperkt tot één specifiek stuk hardware. Bovendien moet een docent ook enorm een enorme inverstering in kennis doen vooraf. 
